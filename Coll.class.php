...
/**
 * WF_Coll 实现了一个类型安全的对象集合
 *
 * WF_Coll 会检查每一个对象的类型是否符合预期，以便将同一类型的对象组织在一起。
 * WF_Coll 具有和 PHP 内置数组相似的性质，因此可以按照使用数组的方式来使用 WF_Coll 集合。
 *
 * 在构造一个集合时，必须指定该集合能够容纳的对象类型：
 *
 * @code php
 * $coll = new WF_Coll('MyObject');
 * $coll[] = new MyObject();
 *
 ...
 * @package core
 */
class WF_Coll implements Iterator, ArrayAccess, Countable
{
    /**
     * 集合对象的类型
     *
     ...
     * 从数组创建一个集合
     *
     * WF_Coll::createFromArray() 方法从一个包含多个对象的数组创建集合。
    ...
    function offsetGet($offset)
    {
        ...
        // LC_MSG: 无效的键名 "%s".
        throw new LogicException(__('无效的键名 "%s".', $offset));
    }
    ...
    function first()
    {
        ...
        // LC_MSG: "%s" 集合中没有任何对象.
        throw new LogicException(__('"%s" 集合中没有任何对象.', $this->_type));
    }
     ...
     * @endcode
     *
     * WF_Coll::append() 在追加数据时不会保持键名。
     *
     * @param array|WF_Coll $data 要追加的数据
     *
     * @return WF_Coll 返回集合对象本身，实现连贯接口
     */
    function append($data)
    {
        ...
        else
        {
            ...
            throw new LogicException(__('"%s()" 的参数 "%s" 必须是 "%s"，但实际提供的是 "%s".',
     ...
     *
     *     // 用于 WF_Coll 的回调方法
     *     static function _WF_Coll_callback()
     *     {
     ...
     * @endcode
     *
     * 当调用 WF_Coll 自身没有定义的方法时，WF_Coll 将认为开发者是要对集合中的每一个对象调用指定方法。
     *
     * -  此时，WF_Coll 首先检查集合中的对象是否提供了 _WF_Coll_callback() 静态方法；
     * -  如果有，则通过 _WF_Coll_callback() 取得一个方法映射表；
     * -  WF_Coll 根据 _WF_Coll_callback() 返回的方法映射表调用对象的其他静态方法。
     * -  如果没有提供 _WF_Coll_callback() 方法，或方法映射表中没有指定的方法。
     *    WF_Coll 则遍历集合中的所有对象，尝试调用对象的指定方法。
     *
     * @param string $method
     * @param array $args;
     *
    ...
    function __call($method, $args)
    {
        ...
        if (method_exists($this->_type, '_WF_Coll_callback'))
        {
            $map = call_user_func(array($this->_type, '_WF_Coll_callback'));
