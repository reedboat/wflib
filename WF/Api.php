...
class WF_Api {
    ...
    {
        $this->beginReport($name);
        $this->boot_time = isset($GLOBALS['g_boot_time']) ? $GLOBALS['g_boot_time'] : microtime(true);
    ...
    {
        return $this->prj->fw->interface->$key;
    }
    ...
    public function validate($rules){
        ...
        if (!$result){
            $msgs = $validator->getMessage('normal');
    ...
    public function beginReport($name)
    {
        if (class_exists('CPhpPerfReporter')){
            $reporter = new CPhpPerfReporter();
            if(preg_match('/^(.+)\.(\w+)\.(\w+)$/', $name, $match)){
                $appName = $match[1];
                $appName = str_replace('.', '_', $appName);
                $reporter->beginPerfReport($appName, null, null, true);
                $reporter->setClassName($match[2]);
                $reporter->setFunctionName($match[3]);
            }
            else {
                $appName = $name;
                $appName = str_replace('.', '_', $appName);
    ...
    public function endReport($retCode)
    {
        if ($this->reporter){
            $this->reporter->endPerfReport($retCode);
        }
    }
}
